...
if not exist "%~dp0tekika-ai-backend\requirements.txt" (
    ...
    if errorlevel 1 (
        ...
        powershell -NoProfile -Command "Write-Host '    ^> Required Python packages are missing.' -ForegroundColor Yellow"
...
if not exist "%~dp0tekika-ai-frontend\package.json" (
    ...
    powershell -NoProfile -Command "Write-Host '    ^> Make sure the Tekika AI frontend files are present.' -ForegroundColor Yellow"
    set /a ERROR_COUNT+=1
) else (
...
if "%ERROR_COUNT%"=="0" (
    ...
    echo.
    echo Tekika AI is ready to launch.
    echo Run start-tekika.bat to start Tekika AI.
) else (
...
if not defined CHECK_KEY (
    powershell -NoProfile -Command "Write-Host '[NG] %~2 API key (%~1) is missing or empty.' -ForegroundColor Red"
    set /a ERROR_COUNT+=1
) else (
